...
1. Open the Google CLoud Shell, copy the command code snippets and run them in sequence in the shell.
   ...
   ```bash
   gcloud iam roles update doit_flexsave_role --organization=$ORGANIZATION_ID --title="DoiT Flexsave Role" --stage=GA --add-permissions=bigquery.jobs.create,billing.resourceAssociations.create,resourcemanager.projects.createBillingAssignment
   ```
