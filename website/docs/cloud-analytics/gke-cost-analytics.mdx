---
sidebar_position: 14
description: How to unlock Google Kubernetes Engine cost data and make it available in Cloud Analytics
---
...
- [Forecast](forecasting.mdx) and [identify trends](trend-analysis.mdx) across any/all K8s primitives in one click.
...
In order to do this, you will need to [connect your Google Cloud Organization](../google-cloud/connect-google-cloud-service-account.mdx) to the CMP. Please navigate to the Settings page via the gear icon in your left-hand navigation panel.

Then check the box(es) of the additional features you'd like to enable, making sure to select "GKE Cost Analytics" here.

![A screenshot showing you the location of the location of the Upload Key button](../../static/img/docs/cmp-gke-cost-analytics-enable.png)
...
In order to do this for each GKE usage dataset you've created, you will need to ensure this role is assigned to the Service Account email created in [Step 1](gke-cost-analytics.mdx#step-1-enable-the-gke-cost-analytics-feature-in-the-doit-cmp) above.

First, navigate to the BigQuery console and locate the dataset you chose from [Step 2](gke-cost-analytics.mdx#step-2-enable-gke-usage-metering-in-the-google-cloud-console) to store the cluster usage information.

From there, select "Share Dataset"

![A screenshot showing you the location of the Share Dataset button](../../static/img/docs/gcp-share-dataset-button.png)
...
![A screenshot showing how to grant the BigQuery Data Viewer role](../../static/img/docs/gcp-grant-bigquery-data-view-role.png)
...
Once everything is set up, your team's GKE metering data can be analyzed using the CMP's [Cloud Analytics](create-cloud-report/index.mdx) functionality.
...
![A screenshot showing the GKE dimensions](../../static/img/docs/cmp-gke-dimensions.png)

You will also find any GKE labels in the Labels chip, under the "GKE Labels" header in the dropdown.

![A screenshot showing the GKE Labels dropdown](../../static/img/docs/cmp-gke-labels-dropdown.png)
