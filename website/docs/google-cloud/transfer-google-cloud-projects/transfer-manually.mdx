...
The highlighted line which shows the `echo` command used to print the `gcloud` command that _would_ be executed, instead of executing it (i.e., it performs a dry run).
...
<dl>
  ...
  <dd>
    ...
    <p>
      Then, for each Project ID, it runs the the <code>echo</code> command
      (and everything that follows it), substituting <code>{'{}'}</code> for
      the Project ID.
    </p>
  </dd>
</dl>
...
Remove the `echo` from the pipeline above to execute the `billing project link` command for every project you have access to:
