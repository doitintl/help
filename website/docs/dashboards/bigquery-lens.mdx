...
- **Refresh**: By default, the BigQuery Spend by SKU widget is updated twice per day. You can use the _Refresh_ option to fetch the latest billing data from Google.
  ...
  The information shown here is when CMP retrieves the billing data, while the timeliness of the data itself is decided by Google Cloud's [frequency of data loads](https://cloud.google.com/billing/docs/how-to/export-data-bigquery-tables#data-loads).

  :::

- **Open full report**: Opens the [report](../cloud-analytics/create-cloud-report/index.mdx) that generates the visualization in this widget.
